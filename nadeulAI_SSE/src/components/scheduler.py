...
class Scheduler:
    ...
    async def scheduling(assigned_transformed_dto: schemas.AssignTransformedDTO) -> str:
        async with Scheduler.lock:
            ...
            busy_log_flag = False
            while True:
                current_ai_server_idx = int(
                    await Scheduler.r_lb.get("current_ai_server_idx")
                )
                ...
                if (
                    await Scheduler.r_lb.get(
                        f"ai_server_is_busy_{current_ai_server_idx}"
                    )
                    is not None
                ):
                    pass
                    ...
                    await Scheduler.r_schedule.set(
                        hash_id,
                        json.dumps(
