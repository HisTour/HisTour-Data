...
class Scheduler():
    ...
    async def scheduling(assigned_transformed_dto:schemas.AssignTransformedDTO) -> str:
        async with Scheduler.lock:
            current_ai_server_idx = await int(Scheduler.r_lb.get("current_ai_server_idx"))
            ...
            while True:
                if int(Scheduler.r_lb.lindex("ai_server_is_busy", current_ai_server_idx)):
                    pass

                else:
                    hash_id = Scheduler.make_hash(current_ai_server_idx, assigned_transformed_dto.character_type)
                    await Scheduler.r_schedule.set(hash_id, json.dumps(assigned_transformed_dto, ensure_ascii=False))
